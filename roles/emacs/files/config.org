...
This is one  of the most useful package when  used with tramp, when it comes  to editing remote
files, or even local  files. It allows editing files as root with  just a keybinding, therefore
preventing the hassle to exit and reopen as sudo.
...
(use-package consult-lsp
  :ensure t)
;; not sure about languages and lsp yet, but let's see later
#+END_SRC
...
some host/server, use the following in the header =:dir /ssh:user@host:/home/printfdebugging/repos= for example. One thing to note is that for
this to work, sshd should be up and running.
...
example.  This doesn't involve sshd, and runs commands with sudo previliges.

=:result <type>= is used  to determine the format of the source-block  output/result. By default it's a table, but there  are other options as
well like =verbatim=, =drawer=, =raw= etc. Out of these, I find raw to be the most annoying, as it's just text and not a part of =#+RESULT:=.
...
(defun goto-org-roam-node-at-point ()
  ...
  (let ((id (extract-org-roam-id-at-point)))
    (if id
      ...
      (message "Failed to extract ID from link at point."))))
...
(use-package dashboard
  ...
  :init
    ...
    (setq dashboard-startup-banner "/home/printfdebugging/.config/emacs/townhall.webp")
...
* TODO Configuring emacs for cpp and game development

Still a lot of things like the code action stuff, and the error reporting things are left.

#+BEGIN_SRC emacs-lisp
;; installing lsp mode and the language mode
...
ERC is a really cool and minimal emacs IRC client. I want to keep it low from now on, no distractions here and there, so I want to keep to
a few chosen groups. I want ERC to authenticate me, and jon all the channels on startup.
...
It's not easy to setup Mu4e in  emacs, and that's why I will make this section into a tutorial on how  to setup mu4e correctly on emacs. I
am on arch linux, and so will be using =pacman= as the package manager.
...
* TODO Tab Overloading

So if the character before the cursor is a space, or start of the line, then =[TAB]= acts as =tab-to-tabstop=, else if there is some character
before it, then it acts as the completion engine. I want a way to add my custom yasnippets to the completion engine such that pressing tab
on the shortword shows the custom snippets on the vertico minibuffer.
